...
int main(int argc, char *argv[])
{
	...
	// Pressure dependence for compressible gas (pressure dependence is moved outside the PIMPLE loop after p is fully resolved)
        if (activateTemperatureDependenceOfProperties == 1)
        {
        	if (activatePressureDependenceOfDensityForWettingPhase == 1)
                {
                    rho1 = max(min(rho1 * (p/pRef), rhoMax), rhoMin);
                }
                if (activatePressureDependenceOfDensityForNonWettingPhase == 1)
                {
                    rho2 = max(min(rho2 * (p/pRef), rhoMax), rhoMin);
                }
	}
